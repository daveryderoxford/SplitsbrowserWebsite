...
require_once("model/sportidentcsvvalidate.php");
                   ...
                   &$errorArray)
{
    ...
    if ($eventId != 0) {
	    $ret = saveResultsFile($eventId, $filename, $format);
        if ($ret==false) {
           deleteEvent($eventId);
           $errorArray[] = "Error encountered saving result file.  Verify format is correct";
           return(FALSE);
        }
	}
...
function updateResults($eventId, $filename, $fileFormat)
{
   ...
   $ret = saveResultsFile($eventId, $filename, $fileFormat);
...
*/
function saveResultsFile($eventId, $filename, $fileFormat)
{
    // Remove unused fields from SportIdent CVS files
    if ($fileFormat=="SICSV") {
        $s = new SportIdentCVSValidator;
        $tempfile = getResultsRoot() . $eventId . ".tmp";
        $ret = $s->stripUnusedFields($filename, $tempfile); 
        if ($ret==false)
        {
            return(false);
        }
